...
@RestController
@RequestMapping(value = ["/api/TunnelClass"], produces = [MediaType.APPLICATION_JSON_VALUE])
class TunnelClassController : EntityClassController() {
    override fun addEntityClass(name: String, alias: String): INodeClass? =
            TunnelClass(generateId()).let {
                it.initialize(null)
